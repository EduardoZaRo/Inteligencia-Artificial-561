'''
3. Diseñar modelos de clasificación logística para detectar cáncer 
a partir de datos de espectrometría de masas en perfiles de 
proteínas (ovarian_dataset.dat) que establecen la relación para 
distinguir entre pacientes con cáncer (targets, últimas dos 
columnas de la tabla) y medidas de intensidad de iones 
(inputs, primeras 100 columnas de la tabla).
La metodología es seleccionar un conjunto reducido de medidas 
o "características" que pueden usarse para distinguir entre 
pacientes con cáncer y de control mediante un clasificador. Estas
características serán niveles de intensidad de iones a valores 
específicos de masa / carga. El objetivo del modelo de 
clasificación es distinguir entre el cáncer y el control de 
los pacientes a partir de los datos de espectrometría de masas.
...
def logitAvgLoss(A, Y, vecX):
    ...
    P = sigmoide(hx)
    e = Y-P
    #Para evitar log(0) se le suma un valor muy pequeño
...
dataset = np.loadtxt('C:\\Users\\irvin\\Desktop\\Meta_5_5\\{}'.format("ovarian_dataset.dat"), delimiter = '\t')
